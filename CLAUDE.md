...
- `FASTBAYES` — per-grid-axis refined values + formal uncertainties + posterior mean/mode/percentiles, plus derived `LOGMSTAR` (surviving stellar mass)/`SFR`/`SSFR`/`T50` (half-mass assembly time)/`MSTARAGE` (mass-weighted age), K-corrections/absolute magnitudes, rest-frame luminosities, and `DN4000_MODEL`; `KCORR*_ERR_*`/`ABSMAG*_SYNTH_ERR_*` (present only when the grid file has a `RESTMAGGIES` extension) add a full-discrete-posterior weighted-variance uncertainty on `KCORR`/the model-only absolute magnitude, and also feed a corrected (no-double-counting) `ABSMAG*_IVAR_*` that combines observational and model uncertainty
