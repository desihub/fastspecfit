...
see \texttt{fastspecfit.templates.continuity\_agebins}) computed with
FSPS's Kriek \& Conroy (2013) dust attenuation curve
(\texttt{dust\_type=4}, with \texttt{dust1=0} and \texttt{dust2}
$\equiv\tau$ as the free diffuse optical depth), reradiated dust emission
...
Dust optical depth & $\tau$ (\texttt{dust2}) & 6 & 0.0, then log-spaced 0.01 to 4.0 \\
Dust index & $n_{\rm dust}$ (\texttt{dust\_index}) & 10 & $-1.5$ to $0.4$ (linear) \\
...
\bottomrule
\end{tabular}
\caption{The eight Bayesian grid axes (\texttt{GRID\_AXIS\_COLUMNS} in
  \texttt{fastspecfit.fastbayes}), in full-factorial nested-loop order.
  Total grid size $6\times4\times6\times10\times1\times1\times3\times4 =
  17{,}280$ models. Axes built log-uniform (age, metallicity, $U_{\rm min}$,
  ...
  the remaining axes ($\tau$, $n_{\rm dust}$, $\gamma$, $f_{\rm AGN}$) are
  refined and reported in linear space -- required for $\tau$ and
  $f_{\rm AGN}$, whose hybrid exact-zero-plus-log-spaced design includes a
  point ($=0$) that $\log_{10}$ cannot represent. The age axis switched
  from a 20-point single-age SSP (\texttt{sfh=0}) grid to a 6-point
  continuity-bin SFH (\texttt{sfh=3}), a more physically realistic star
  formation history that already marginalizes over the ages within each
  bin, needing fewer points; $\tau$ likewise dropped from a 12-point
  linear-uniform grid to a 6-point exact-zero-plus-log-spaced grid, better
  matching the physical clustering of galaxies toward low-to-moderate
  optical depth. Both changes free grid budget spent elsewhere: $n_{\rm
  dust}$ (also well constrained by the UV/optical/NIR SED shape alone, and
  partially degenerate with $\tau$) gained the resolution $\tau$ gave up
  (8 to 10 points); $U_{\rm min}$ and $\gamma$ (essentially unconstrained
  without far-IR photometry, per Leja et al. 2017 App.~C) collapsed from a
  coarse $2\times2$ grid to single fixed values, freeing budget for the new
  $f_{\rm AGN}$ axis (with \texttt{agn\_tau} likewise fixed, at FSPS's own
  default); $Q_{\rm PAH}$ (constrained by mid-IR photometry alone) kept
  its previous resolution unchanged.}
\label{tab:grid}
\end{table}
...
17{,}280 templates), giving the discrete maximum-likelihood template
...
reported for every parameter $x$ --- the eight grid axes, $\log_{10}
...
the eight grid axes and the model $D_n(4000)$ index (intensive quantities,
...
N-linear interpolation over the $2^8=256$ grid corners bracketing the
refined point: corner $\kappa$, defined by one bit per axis, carries weight
\begin{equation}
  w_\kappa \;=\; \prod_{a=1}^{8} \bigl[\kappa_a\, f_a + (1-\kappa_a)(1-f_a)\bigr],
  \qquad \kappa_a \in \{0, 1\},
  \label{eq:corner_weight}
\end{equation}
computed for all 256 corners at once via array broadcasting over the bit
