...
attenuation component, and \texttt{dust2} $\equiv\tau_V$ as the sole free
...
\caption{The seven axes of the bundled default Bayesian grid
  ...
  the remaining axes ($\tau_V$, $\gamma$, $f_{\rm AGN}$) are refined and
  reported in linear space -- required for $\tau_V$ and $f_{\rm AGN}$, whose
  ...
  $\tau_V$ and $n_{\rm dust}$ dangerously degenerate/covariant and poorly
  ...
  in finer $\tau_V$ sampling (6 to 10 points) and metallicity (4 to 5
  points), with the remainder left unspent for now. $\tau_V$'s grid points
  ...
  continuous prior shape (peaked at zero, falling off toward high $\tau_V$,
...
spectrum used for the derived quantities in Section~\ref{sec:derived}. This
spectrum is not itself persisted in the output file: it is cheap and
deterministic to rebuild on demand (e.g. for QA) from the stored observed
photometry, redshift, and photometric system alone.
