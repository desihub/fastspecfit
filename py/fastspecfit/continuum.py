...
class ContinuumTools(object):
    ...
    def smooth_continuum(self, wave, flux, ivar, redshift, medbin=120, medpad=30,
        ...
        :class:`numpy.ndarray` [npix]
            Smooth continuum which can be subtracted from `flux` in order to
            create a pure emission-line spectrum.
            ...
            if np.sum(inrange) > 0:
                ...
                if len(stackflux) > 0: 
                    ...
                    if len(stackflux) > 10: # require at least 10 pixels
                        ...
                        if ax:
                            _label = '{}\n sigma={:.0f} km/s\n S/N={:.1f}'.format(label, linesigma, snr)
                            ax.scatter(stackdvel, stackflux, s=10, label=_label)
                            if popt is not None:
                                srt = np.argsort(stackdvel)
                                ax.plot(stackdvel[srt], onegauss(stackdvel[srt], *popt), color='k')#, label='Gaussian Model')
    ...
    def build_linemask(self, wave, flux, ivar, redshift=0.0, nsig=5):
    ...
    def everest_build_linemask(self, wave, flux, ivar, redshift=0.0):
        ...
        def _estimate_linesigma(zlinewaves, sigma, label='Broad-line', junkplot=None):
            ...
            if np.sum(inrange) > 0:
                ...
                for zlinewave in zlinewaves[inrange]:
                    I = ((wave >= (zlinewave - 4*sigma * zlinewave / C_LIGHT)) *
                         (wave <= (zlinewave + 4*sigma * zlinewave / C_LIGHT)))
                    if np.max(flux[I]) > 0:
                        stackdvel.append((wave[I] - zlinewave) / zlinewave * C_LIGHT)
        ...
        # Hbeta, Halpha
        #zlinewaves = np.array([6564.613]) * (1 + redshift)
        zlinewaves = np.array([4862.683, 6564.613]) * (1 + redshift)
        ...
        if linesigma_balmer >= self.linemask_sigma_balmer:
            self.linemask_sigma_balmer = 2*linesigma_balmer
        #if linesigma_balmer >= self.linemask_sigma_balmer:
        #    linesigma_balmer, narrow_balmer = _estimate_linesigma(
        #        zlinewaves, 2*self.linemask_sigma_balmer, label='Balmer-line', junkplot='/global/homes/i/ioannis/desi-users/ioannis/tmp/junk-balmer.png')
            
        if (linesigma_balmer < 50) or (linesigma_balmer > 3000) or (narrow_balmer < 3):
