...
class TabulatedDESI(object):
    ...
    def __repr__(self):
        return f'TabulatedDESI(file={self.file})'
...
class FlatLambdaCDM(object):
    r"""Analytic flat :math:`\Lambda\mathrm{CDM}` cosmology (matter and dark
    energy only), fixed to :math:`h=1`.

    A lightweight, dependency-free alternative to :class:`TabulatedDESI` for
    one-off projects that need a different cosmology (e.g., a round
    :math:`\Omega_M=0.3`). :math:`E(z)` is evaluated analytically; comoving
    distance is pre-tabulated at construction time on the same redshift grid
    used to build :class:`TabulatedDESI`'s table, so
    :meth:`comoving_radial_distance` stays a fast interpolation rather than
    a per-call numerical integration.
    ...
    ----------
    H0 : float
        Hubble constant in km/s/Mpc (fixed at 100, i.e., :math:`h=1`).
    h : float
        Dimensionless Hubble parameter (fixed at 1).
    hubble_time : float
        Hubble time in Gyr.

    Notes
    -----
    Redshift interpolation range for :meth:`comoving_radial_distance` is
    [0, 100]; :meth:`efunc` is evaluated analytically and is not restricted
    to that range.
    ...
    def __init__(self, omega_m=0.3):
        ...
        self._z = np.hstack((0., np.logspace(-8, 2, 40001)))
        self._efunc = self._efunc_analytic(self._z)

        hubble_distance = C_LIGHT / self.H0 # Mpc/h (=Mpc, since h=1)
        self._comoving_radial_distance = hubble_distance * cumulative_trapezoid(
            1. / self._efunc, self._z, initial=0.)


    def __repr__(self):
        return f'FlatLambdaCDM(omega_m={self.omega_m}, h={self.h})'


    def _efunc_analytic(self, z):
    ...
    def efunc(self, z):
        ...
        """
        return self._efunc_analytic(np.asarray(z))
    ...
    def universe_age(self, z):
        ...
        if np.isscalar(z):
            integ, _ =  quad(_agefunc, z, np.inf)
            return integ * self.hubble_time
        else:
            age = []
            for _z in z:
